...
def show_dashboard():
    ...
    book_parking_button = tk.Button(sidebar_frame, text="Book Parking Lot", font=("Arial", 12), bg="white", command=show_book_parking)
    book_parking_button.pack(pady=10, padx=10, fill=tk.X)

    booking_history_button = tk.Button(sidebar_frame, text="Booking History", font=("Arial", 12), bg="white", command=show_booking_history)
...
def show_book_parking():
    ...
    # Select button
    select_button = tk.Button(content_frame, text="Select", font=("Arial", 14), command=lambda: show_parking_spots(parking_lot_dropdown.get()))
    select_button.pack(pady=10)

def show_parking_spots(location):
    ...
    if parking_lot:
        ...
        for i in range(total_spots):
            spot_number = i + 1
            spot_button = tk.Button(spots_frame, text=str(spot_number), font=("Arial", 14), bg="green", width=4, height=2, command=lambda spot=spot_number: select_spot(spot))
...
def select_spot(spot_number):
    ...
    spot_button = content_frame.winfo_children()[1].winfo_children()[spot_number - 1]
...
def show_booking_history():
    ...
    for booking in bookings:
        ...
        exit_button = tk.Button(content_frame, text="Exit", font=("Arial", 12), command=lambda booking_id=booking_id: exit_parking(booking_id))
        invoice_button = tk.Button(content_frame, text="View Invoice", font=("Arial", 12), command=lambda booking_id=booking_id: view_invoice(booking_id))

        if exit_time:
            total_amount = amount_paid + (additional_amount or 0)
            booking_history_tree.insert("", tk.END, values=(location, spot_number, duration_hours, f"₹{amount_paid:.2f}", "-", invoice_button))
        else:
            booking_history_tree.insert("", tk.END, values=(location, spot_number, duration_hours, f"₹{amount_paid:.2f}", exit_button, invoice_button))
